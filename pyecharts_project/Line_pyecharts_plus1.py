...
line_style_1 = opts.LineStyleOpts(
    width=3,
    type_='solid',  # 实线
    # color='#FF5733'
)

line_style_2 = opts.LineStyleOpts(
    width=3,
    type_='dashed',  # 虚线
    # color='#33FF57'
)
...
line = (
    ...
    .add_yaxis('平均在线人数', [item[1]/1000 for item in data_pair[::-1]],
               ...
               linestyle_opts=line_style_1) #应用自定义样式
    .add_yaxis('峰值在线人数', [item[2]/1000 for item in data_pair[::-1]],
               is_smooth=True, #平滑曲线
               is_symbol_show=False, #不显示数据点符号
               linestyle_opts=line_style_2) #应用自定义样式
    .set_global_opts(
...
out_file = out_dir / 'Line_pyecharts_plus1.html'
